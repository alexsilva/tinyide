...
describe("folded editor pointer mapping", () => {
  it("maps browser-resolved projected offsets back to source offsets", () => {
    expect(pointerSource).toContain("function editorSourceOffsetFromProjectedOffset(");
    expect(pointerSource).toContain("const projectedPosition = textPositionAtOffset(projection.content, rawProjectedOffset)");
    expect(pointerSource).toContain("function editorProjectedTextOffsetAtClientPoint(");
    expect(pointerSource).toContain("projection.fileLineByVisibleLine[visibleLineIndex]");
    expect(pointerSource).toContain("projection.foldIdByMarkerVisibleLine.get(projectedPosition.line)");
  });
  ...
  it("prefers the rendered syntax layer for both left click correction and context menus", () => {
    ...
    expect(pointerSource).toContain("editorMirrorTextOffsetAtClientPoint(textarea, mirror, clientX, clientY)");
    expect(pointerSource).toContain("editorSourceOffsetFromProjectedOffset(textarea.value, projection, mirrorOffset)");
  ...
  it("remaps folds after visible edits instead of clearing every fold", () => {
    expect(foldingSource).toContain("function remapDocumentFoldsAfterEdit(");
  ...
  it("positions the projected caret on empty visual lines instead of the previous line", () => {
    expect(pointerSource).toContain("if (lineStart === lineEnd) {");
    expect(pointerSource).toContain("(base.startLine - 1 + position.line - 1) * lineHeight");
    expect(pointerSource).toContain("Math.max(0, (lineHeight - height) / 2)");
  });
